 ...
 * Problem E: Little Elephant and Mouses
 * Algorithm: DP, O(NM)
...
int main() {
  ...
  while (t--) {
    ...
    FORD(i, n - 1, 0)
      FORD(j, m - 1, 0) {
        dp[i][j][0] = dp[i][j][1] = check(i - 1, j) + check(i, j - 1);
        if (i == n - 1)
          if (j < m - 1) {
            dp[i][j][0] += dp[i][j + 1][0];
            dp[i][j][1] = INF;
          } else {
            dp[i][j][0] = dp[i][j][1] = dp[i][j][0] + check(i, j);
          }
        else if (j == m - 1) {
          dp[i][j][1] += dp[i + 1][j][1];
          dp[i][j][0] = INF;
        } else {
          dp[i][j][0] += min(dp[i][j + 1][1], dp[i][j + 1][0] + check(i + 1, j));
          dp[i][j][1] += min(dp[i + 1][j][0], dp[i + 1][j][1] + check(i, j + 1));
        }
